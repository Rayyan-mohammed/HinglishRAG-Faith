...
**Week:** 2
**Problem:** Running `scripts/generate_answers.py` (A2) against all 60 eval questions hit Groq's free/on-demand tier limit of 100,000 tokens per day (TPD) partway through — the run stopped at question 56/60 with a `RateLimitError`. This is a daily quota, not a short-term rate limit, so retrying every 30s for 6 minutes didn't help; the error message's "try again in Nm" countdown refers to when the rolling 24h window frees up enough tokens, not an imminent reset.
**Fix:** Made `scripts/generate_answers.py` resumable — it now reads any existing `results/generated_answers.csv`, skips `question_id`s already present, and only generates the missing ones. This let the 56 completed answers be committed as progress instead of being discarded or re-generated (re-spending tokens) on every retry. The remaining questions (57–60, all PM Awas Yojana / documents category) get filled in by re-running the same script once the daily quota resets.
**Lesson:** For any future full-batch run against the Groq free tier (e.g. B3's per-claim verification pass in Week 3, which will multiply the number of calls by the average claims-per-answer), budget for hitting this same 100k-tokens/day ceiling and design the driving script to be resumable from the start, not after the first failure.
